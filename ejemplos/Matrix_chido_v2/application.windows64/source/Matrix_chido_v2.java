...
public class Matrix_chido_v2 extends PApplet {
...
public void pintaLetr(int[] _userMap, int _index, int letraS, PVector _realWorldPoint) {
  //
  ...
  textSize(letraS);
  text(letters[PApplet.parseInt(random(letters.length-1))], _realWorldPoint.x, _realWorldPoint.y, _realWorldPoint.z);
}

public void pintaLetr(int letraS, PVector _realWorldPoint) {
...
public void matrix(float xHnd, float yHnd) {
  ...
  if (true) {
    ...
    for (int i = 0; i<stars.size (); i+=2) {
      ...
      /* movement+"glitter"*/
      stars.set(i, new PVector(x-map(xHnd, 0, width, -0.05f, 0.05f)*(w2-x), y-map(yHnd, 0, height, -0.05f, 0.05f)*(h2-y), d + 0.2f - 0.6f * noise(x, y, frameCount)));

      if (d>3||d< -3) stars.set(i, new PVector(x, y, 1));
      if (x<0||x>width||y<0||y>height) stars.remove(i);
      if (stars.size()>9999) stars.remove(1);

      pintaLetr(PApplet.parseInt(d)*10, new PVector(x, y, d));
...
//}
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--present", "--window-color=#666666", "--stop-color=#cccccc", "Matrix_chido_v2" };
    if (passedArgs != null) {
